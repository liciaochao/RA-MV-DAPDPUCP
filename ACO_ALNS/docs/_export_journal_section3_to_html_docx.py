...
def parse_markdown(md_text: str) -> list[dict[str, object]]:
    ...
    for raw in lines + [""]:
        ...
        if stripped[:2].isdigit() and len(stripped) > 2 and stripped[2] == ".":
            flush_paragraph()
...
<title>Section 3 Offline Algorithm (Journal Style)</title>
<style>
body {{
  margin: 0;
  background: #f3f5f7;
...
h3 {{ margin-top: 1.2rem; font-size: 1.2rem; }}
...
def make_docx_paragraph(text: str, style: str = "Normal", preserve: bool = False) -> str:
    if text == "":
        return "<w:p/>"
    p_style = f"<w:pPr><w:pStyle w:val=\"{escape(style)}\"/></w:pPr>"
        ...
        if btype == "heading":
            ...
            if not code_text:
                ...
                for ln in code_text.splitlines():
                    body.append(make_docx_paragraph(ln, style="Code", preserve=True))
...
<cp:coreProperties xmlns:cp="http://schemas.openxmlformats.org/package/2006/metadata/core-properties"
  ...
  <dc:title>paper_section3_offline_algorithm_journal</dc:title>
...
def main() -> None:
    ...
    src_path = docs_dir / "paper_section3_offline_algorithm_journal_source.md"
    html_path = docs_dir / "paper_section3_offline_algorithm_journal.html"
    docx_path = docs_dir / "paper_section3_offline_algorithm_journal.docx"
